...
def run_experiment():
    ...
    # Padding token handling - only filter if pad_token_id is actually defined
    ...
    if pad_token_id is None:
        ...
        print("  This is expected for GPT-NeoX/Pythia models.")
    ...
    while total_tokens < n_tokens:
        ...
        if should_filter_padding:
            has_pad = (unique_toks == pad_token_id).any().item()
            if has_pad:
                # Note: This should be rare or never happen for models without padding in their data pipeline
                pass
    ...
    def flatten_data_filtered(indices, s_embed, s_layer0, s_tokens, pad_id, should_filter):
        # We need to filter out padding tokens if pad_id is valid and should_filter is True
        batch_X = []
        batch_L0 = []
        batch_tokens = []
    ...
    print("="*60)
    print(f"{'Feature ID':<12} {'Name':<40} {'Filt':<8} {'Unfilt':<8} {'Diff':<8}")
    print("-" * 80)
    
    # Show first 10 features plus any named features
    named_features = [29007, 29004, 27861, 17939, 22326, 249]
    features_to_show = []
    for fid in named_features:
        if fid in df_comparison['feature_id'].values:
            features_to_show.append(fid)
    
    # Add first few numeric features
    remaining = [fid for fid in df_comparison['feature_id'].values[:10] if fid not in features_to_show]
    ...
    for fid in features_to_show:
        ...
        print(f"{fid:<12} {name:<40} {auc_filt:.4f}   {auc_unfilt:.4f}   {diff:+.4f}")
